...
class ImageAugmenter:
    ...
    def __init__(self, size, class_count):
        ...
        self._ph_original = tf.placeholder(tf.float32, [size[0], size[1], 1])#grayscale
    ...
    def augment_dataset(self, dataset, method=None):
        ...
        for ori, seg in zip(dataset.images_original, dataset.images_segmented):
            ...
            #print(ori_processed.shape)#(128,128,1)
            input_processed.append(ori_processed)
            input_processed_size.append((ori_processed[0].size, ori_processed[1].size))
            #print(ori_processed[0].size)#128
            output_processed.append(seg_processed)
            #print(np.asarray(input_processed).shape)
            #print(np.asarray(input_processed_size).shape)
        return ld.DataSet(np.asarray(input_processed), np.asarray(output_processed), dataset.palette, input_processed_size)
    ...
    def augment(self, image_in, image_out, method=None):
        ...
        if image_in.ndim != 3:
            image_in = image_in[:, :, np.newaxis]#plus
